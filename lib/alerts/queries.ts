...
async function attachNudge(supabase: Client, rows: AlertRow[]): Promise<AlertWithNudge[]> {
  ...
  const checkIds = [...new Set(rows.map((a) => a.source_check_id))];
  const resolverIds = [...new Set(rows.map((a) => a.resolved_by).filter((id): id is string => Boolean(id)))];

  const [vendorsRes, checksRes, paymentsRes, resolversRes] = await Promise.all([
    supabase.from("vendors").select("id, name").in("id", vendorIds),
    supabase.from("verification_checks").select("id, status_value").in("id", checkIds),
  ...
  const statusValueByCheckId = new Map((checksRes.data ?? []).map((c) => [c.id, c.status_value]));
